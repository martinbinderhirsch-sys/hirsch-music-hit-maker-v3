...
(function() {
  ...
  window.lyricsUndo = function() {
    ...
    if(!el) return;
    _redoStack.push(_lastSaved);
    _lastSaved = _undoStack.pop();
    el.textContent = _lastSaved;
    updateUndoButtons();
    showToast(currentLang==='de'?'↩ Rückgängig':'↩ Undo');
  };
  ...
  window.lyricsRedo = function() {
    ...
    if(!el) return;
    _undoStack.push(_lastSaved);
    _lastSaved = _redoStack.pop();
    el.textContent = _lastSaved;
    updateUndoButtons();
    showToast(currentLang==='de'?'↪ Wiederherstellen':'↪ Redo');
  };
  
  // Intercept addLyricsVersion to track undo
  const _origAdd = window.addLyricsVersion;
  window.addLyricsVersion = function(text) {
    pushUndo(text);
    if(typeof _origAdd === 'function') return _origAdd(text);
  };
  
  // Add undo/redo buttons to lyrics result header
  document.addEventListener('DOMContentLoaded', function() {
    setTimeout(() => {
      const titleActions = document.querySelector('.lyrics-result .card-title-actions');
      if(titleActions && !document.getElementById('undo-btn')) {
        const undoBtn = document.createElement('button');
        undoBtn.id = 'undo-btn';
        undoBtn.className = 'btn btn-xs btn-outline';
        undoBtn.title = 'Rückgängig (Ctrl+Z)';
        undoBtn.disabled = true;
        undoBtn.innerHTML = '↩';
        undoBtn.onclick = lyricsUndo;
        
        const redoBtn = document.createElement('button');
        redoBtn.id = 'redo-btn';
        redoBtn.className = 'btn btn-xs btn-outline';
        redoBtn.title = 'Wiederherstellen (Ctrl+Y)';
        redoBtn.disabled = true;
        redoBtn.innerHTML = '↪';
        redoBtn.onclick = lyricsRedo;
        
        titleActions.insertBefore(redoBtn, titleActions.firstChild);
        titleActions.insertBefore(undoBtn, titleActions.firstChild);
      }
    }, 500);
  });
  ...
  document.addEventListener('keydown', function(e) {
    if((e.ctrlKey || e.metaKey) && e.key === 'z' && !e.shiftKey) {
      ...
      if(active && active.id === 'lyrics-output') { e.preventDefault(); lyricsUndo(); }
    }
    if((e.ctrlKey || e.metaKey) && (e.key === 'y' || (e.key === 'z' && e.shiftKey))) {
      const active = document.activeElement;
      if(active && active.id === 'lyrics-output') { e.preventDefault(); lyricsRedo(); }
    }
  });
})();
